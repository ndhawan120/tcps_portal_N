...
export default function RegistrationApproval() {
  ...
  const [users, setUsers] = useState<PendingUser[]>([]); const [loading, setLoading] = useState(true); const [loadingId, setLoadingId] = useState<string | null>(null); const [error, setError] = useState<string | null>(null);
  const load = async () => {
    setLoading(true); setError(null);
    const response = await fetch("/api/admin/registrations", { cache: "no-store" });
    const result = await response.json().catch(() => ({}));
    if (!response.ok) setError(result.error || "Unable to load registrations."); else setUsers(result.users ?? []);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  const decide = async (userId: string, status: "active" | "rejected") => {
    setLoadingId(userId); setError(null);
    const response = await fetch(`/api/admin/registrations/${userId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) });
    const result = await response.json().catch(() => ({}));
    setLoadingId(null);
    if (!response.ok) { setError(result.error || "Unable to process registration."); return; }
