...
export default async function TeamPage() {
  ...
  return <div><RealtimeRefresh /><Nav role={profile.role} name={`${profile.first_name ?? ""} ${profile.last_name ?? ""}`} /><main className="max-w-7xl mx-auto px-6 py-8">
    <div className="flex items-center justify-between mb-1"><div><h1 className="text-2xl font-bold text-on-surface">{isAdmin ? "Organisation Reporting" : "Team Overview"}</h1><p className="text-sm text-on-surface-variant">{isAdmin ? "Overall reporting across active employees." : "Only your immediate active team is included."}</p></div><Link href="/employees" className="text-sm font-semibold text-primary hover:underline">{isAdmin ? "People →" : "My Team →"}</Link></div>
    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 my-6"><MetricCard label={isAdmin ? "Employees" : "Team Members"} value={teamMembers.length} /><MetricCard label="Average PER Progress" value={`${avgProgress}%`} /><MetricCard label="Pending PER Approvals" value={perCounts.pending_approval ?? 0} /><MetricCard label="Exam Progress" value={`${examOverallProgress}%`} /></div>
