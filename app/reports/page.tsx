...
export default async function ReportsPage() {
  ...
  return <div className="min-h-screen bg-[#f9f9f9]"><Nav role={profile.role} name={`${profile.first_name ?? ""} ${profile.last_name ?? ""}`} /><main className="lg:ml-64 pt-[92px] px-4 sm:px-6 pb-10"><div className="max-w-7xl mx-auto space-y-7">
    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4"><div><h1 className="text-3xl font-extrabold">{profile.role === "admin" ? "Organisation Reports" : "Team Reports"}</h1><p className="text-sm text-on-surface-variant mt-1">{profile.role === "admin" ? "Organisation-wide performance analysis for active employees." : "Performance analysis for your assigned employees only."}</p></div><Link href="/team" className="text-sm font-semibold text-primary hover:underline">Back to {profile.role === "admin" ? "Workforce" : "Team"} →</Link></div>
