...
export default async function ExamsPage() {
  ...
  const [{ data: exams, error: examsError }, { data: objectives, error: objectivesError }, { data: history, error: historyError }] = await Promise.all([
    ...
    supabase.from("approval_history").select("id, objective_id, action, comments, created_at, actor_id").in("objective_id", (await supabase.from("per_objectives").select("id").eq("user_id", user.id)).data?.map((row) => row.id) ?? []).order("created_at", { ascending: false }),
  ]);
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface">
      ...
      <main className="lg:ml-64 pt-0 lg:pt-[68px]">
        {(examsError || objectivesError || historyError) && (
        ...
        <ExamsTracker userId={user.id} exams={exams ?? []} objectives={objectives ?? []} history={history ?? []} />
      </main>
    </div>
  );
}
