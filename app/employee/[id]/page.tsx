...
export default async function EmployeeDetailsPage({
  ...
  // A manager may only access an employee who is currently assigned to them.
  // This check is server-side, so changing the URL cannot expose another team.
  ...
  // Use the same 13-paper ACCA completion calculation used by reporting.
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div className="flex items-start gap-4">
              {employee.avatar_url ? (
                ...
                <div className="w-16 h-16 rounded-full bg-surface-container flex items-center justify-center text-lg font-bold text-on-surface-variant">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <ProgressCard
            label="Exam Progress"
            value={`${examProgress}%`}
            sub={`${passedExams}/${TOTAL_EXAMS} counted as passed`}
          />
          <ProgressCard
            label="Pending PER"
