...
export default async function EmployeeProfilePage({ params }: PageProps) {
  ...
  // Profile pages are addressed only by the public-friendly profile_slug.
  // UUIDs are database identifiers and are deliberately not accepted as routes.
  const looksLikeUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(slug);
