...
export default async function AdminPage() {
  ...
  return <div><RealtimeRefresh /><Nav role="admin" name={`${profile.first_name ?? ""} ${profile.last_name ?? ""}`} /><main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
      ...
      <AdminLink href="/approvals" title="Approvals" icon="fact_check" description="Review PER approvals and, for admins, pending registration requests." />
      <AdminLink href="/reports" title="Reports" icon="assessment" description="Open organisation-wide analytics and generate employee reports." />
      <AdminLink href="/admin/roles" title="Roles & Access" icon="manage_accounts" description="Manage user roles and active departments." />
      <AdminLink href="/admin/branding" title="Branding" icon="palette" description="Manage the portal company and logo branding." />
      <AdminLink href="/announcements" title="Notice Board" icon="campaign" description="Publish TCPS notices and view synced ACCA announcements." />
    </section>
    <section className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 mb-6"><div className="flex items-center justify-between gap-4"><div><h2 className="text-lg font-bold text-on-surface">Pending registrations</h2><p className="text-sm text-on-surface-variant mt-1">New accounts waiting for administrator action. The full approval workflow is also available from Approvals.</p></div><Link href="/approvals" className="text-sm font-semibold text-primary hover:underline">{pendingUsers?.length ?? 0} pending →</Link></div><RegistrationApproval /></section>
    <section className="bg-primary/5 border border-primary/20 rounded-xl p-5"><h2 className="text-sm font-bold uppercase tracking-widest text-primary">Where information belongs</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 text-sm"><div><p className="font-semibold text-on-surface">Dashboard</p><p className="text-on-surface-variant mt-1">Summary and next actions, not configuration records.</p></div><div><p className="font-semibold text-on-surface">Admin</p><p className="text-on-surface-variant mt-1">Roles, departments, branding and access controls.</p></div><div><p className="font-semibold text-on-surface">People / Team</p><p className="text-on-surface-variant mt-1">People records and workforce progress.</p></div></div></section>
  </main></div>;
}
