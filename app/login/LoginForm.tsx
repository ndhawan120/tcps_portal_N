...
export default function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (profile.status === "pending" || profile.status === "rejected" || profile.status === "inactive") {
      await supabase.auth.signOut();
      setLoading(false);
      router.push(`/login?error=account_${profile.status}`);
      router.refresh();
      return;
    }

    // last_login is deliberately non-blocking: a reporting timestamp must
    // never prevent a valid employee from signing in.
    await supabase
      .from("profiles")
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-outline-variant">
        ...
        <p className="text-sm text-on-surface-variant mb-6">Professional Development Portal</p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-md border border-outline-variant px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
          <div>
            ...
            <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-md border border-outline-variant px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>

          {error && <p className="text-sm text-error">{error}</p>}

          <button type="submit" disabled={loading} className="w-full bg-primary text-on-primary rounded-md py-2 text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-50">
