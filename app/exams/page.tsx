...
export default async function ExamsPage({ searchParams }: { searchParams?: { search?: string } }) {
  ...
  return <div className="min-h-screen bg-surface text-on-surface"><Nav role={profile.role} name={`${profile.first_name ?? ""} ${profile.last_name ?? ""}`.trim()} /><main className="lg:ml-64 pt-0 lg:pt-[68px]">{(examsError || objectivesError || historyError) && <div className="mx-4 sm:mx-6 lg:mx-10 mt-5 rounded-lg border border-error/30 bg-error-container/40 px-4 py-3 text-sm text-error">Some qualification data could not be loaded. Please refresh and try again.</div>}<ExamsTracker userId={user.id} joiningDate={profile.joining_date} searchQuery={searchParams?.search ?? ""} exams={exams ?? []} objectives={objectives ?? []} history={history ?? []} /></main></div>;
}
