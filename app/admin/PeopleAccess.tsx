...
export default function PeopleAccess({ people, managers }: { people: Person[]; managers: Manager[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return people.filter((person) => {
      ...
      const matchesSearch = !q || name.includes(q) || (person.email ?? "").toLowerCase().includes(q) || (person.department ?? "").toLowerCase().includes(q);
  ...
  return <>
    <div className="px-5 pb-4 flex flex-col lg:flex-row gap-3">
      ...
      {(query || roleFilter !== "all" || statusFilter !== "all") && <button type="button" onClick={() => { setQuery(""); setRoleFilter("all"); setStatusFilter("all"); }} className="rounded-md border border-outline-variant px-3 py-2 text-sm font-medium">Clear</button>}
    </div>
