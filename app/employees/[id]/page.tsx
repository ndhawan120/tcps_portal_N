...
export default async function EmployeeProfilePage({ params }: PageProps) {
  ...
  const employeeQuery = supabase.from("profiles").select("*").eq(looksLikeUuid ? "id" : "profile_slug", identifier).single();
  ...
  return <div>
    <Nav role={viewer.role} name={`${viewer.first_name ?? ""} ${viewer.last_name ?? ""}`.trim()} />
    <main className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 mb-6"><div className="flex flex-col md:flex-row md:items-center gap-5">
        ...
        <a href={`/employees/export?user_id=${employee.id}`} className="inline-flex items-center justify-center text-xs font-semibold px-4 py-2 rounded-md border border-outline-variant text-on-surface hover:bg-surface-container">Download Excel</a>
      </div></div>
