...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(slug)) return { title: "Employee Profile", robots: { index: false, follow: false } };
  const { data: employee } = await supabase.from("profiles").select("first_name,last_name,job_title,department,profile_slug").eq("profile_slug", slug).single();
  if (!employee) return { title: "Employee Profile", description: "Employee professional development profile.", robots: { index: false, follow: false } };
  const name = `${employee.first_name ?? ""} ${employee.last_name ?? ""}`.trim();
  return { title: `${name} | Employee Profile`, description: `${name}'s professional development profile, including ACCA exam and PER progress.`, alternates: { canonical: `/employees/${employee.profile_slug}` }, robots: { index: false, follow: false } };
}
...
export default async function EmployeeProfilePage({ params }: PageProps) {
  ...
  const slug = decodeURIComponent(params.slug).trim().toLowerCase();
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(slug)) notFound();
  const { data: employee, error: employeeError } = await supabase.from("profiles").select("*").eq("profile_slug", slug).single();
  ...
  return <div><Nav role={viewer.role} name={`${viewer.first_name ?? ""} ${viewer.last_name ?? ""}`.trim()} /><main className="max-w-7xl mx-auto px-6 py-8"><div className="mb-6"><Link href="/employees" className="text-xs font-medium text-primary hover:underline">← Back to Employees</Link></div><div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 mb-6"><div className="flex flex-col md:flex-row md:items-center gap-5">{employee.avatar_url ? <img src={employee.avatar_url} alt="" className="w-20 h-20 rounded-full object-cover border border-outline-variant" /> : <div className="w-20 h-20 rounded-full bg-surface-container flex items-center justify-center text-xl font-bold text-on-surface-variant">{getInitials(employee.first_name, employee.last_name)}</div>}<div className="flex-1"><div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-bold text-on-surface">{employee.first_name} {employee.last_name}</h1><EmployeeStatus status={employee.status} /></div><p className="text-sm text-on-surface-variant mt-1">{employee.email}</p><div className="flex flex-wrap gap-x-5 gap-y-1 mt-3 text-xs text-on-surface-variant"><span>Department: <strong className="text-on-surface">{employee.department || "—"}</strong></span><span>Role: <strong className="text-on-surface capitalize">{employee.role || "—"}</strong></span>{employee.job_title && <span>Job title: <strong className="text-on-surface">{employee.job_title}</strong></span>}{employee.joining_date && <span>Joining date: <strong className="text-on-surface">{new Date(employee.joining_date).toLocaleDateString()}</strong></span>}{employee.phone && <span>Phone: <strong className="text-on-surface">{employee.phone}</strong></span>}</div></div><div className="flex flex-col items-end gap-3"><div className="rounded-lg border border-outline-variant bg-surface-container-low px-4 py-3 min-w-[220px]"><p className="text-[11px] font-bold uppercase tracking-wide text-on-surface-variant">PER Objective Number</p><p className="text-lg font-bold text-primary mt-1">{employee.per_objective_number || "Not entered"}</p><p className="text-[11px] text-on-surface-variant mt-1">Employee-maintained reference</p></div><a href={`/employees/export?user_id=${employee.id}`} className="inline-flex items-center justify-center text-xs font-semibold px-4 py-2 rounded-md border border-outline-variant text-on-surface hover:bg-surface-container">Download Excel</a></div></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8"><ProgressCard title="PER Progress" completed={approvedObjectives} total={TOTAL_OBJECTIVES} percentage={perProgress} /><ProgressCard title="Exam Progress" completed={passedExams} total={TOTAL_EXAMS} percentage={examProgress} /></div><section className="mb-8"><h2 className="text-lg font-bold text-on-surface mb-4">PER Status</h2><div className="grid grid-cols-2 md:grid-cols-5 gap-3"><MiniStat label="Not Started" value={notStartedObjectives} /><MiniStat label="Draft" value={draftObjectives} /><MiniStat label="Pending Approval" value={pendingObjectives} /><MiniStat label="Approved" value={approvedObjectives} /><MiniStat label="Rejected" value={rejectedObjectives} /></div></section><section className="mb-8"><h2 className="text-lg font-bold text-on-surface mb-4">Exam Status</h2><div className="grid grid-cols-2 md:grid-cols-5 gap-3"><MiniStat label="Not Started" value={notStartedExams} /><MiniStat label="In Progress" value={inProgressExams} /><MiniStat label="Scheduled" value={scheduledExams} /><MiniStat label="Passed" value={passedExams} /><MiniStat label="Failed" value={failedExams} /></div></section><section className="mb-10"><div className="flex items-center justify-between mb-4"><div><h2 className="text-xl font-bold text-on-surface">PER Objectives</h2><p className="text-xs text-on-surface-variant mt-1">Read-only view of this employee&apos;s PER progress.</p></div><a href={`/employees/export?user_id=${employee.id}`} className="text-xs font-semibold text-primary hover:underline">Export employee data</a></div><ObjectivesList userId={employee.id} existingByNumber={existingByNumber} readOnly /></section><section><div className="flex items-center justify-between mb-4"><div><h2 className="text-xl font-bold text-on-surface">ACCA Exams</h2><p className="text-xs text-on-surface-variant mt-1">Read-only view of this employee&apos;s exam progress.</p></div><a href={`/employees/export?user_id=${employee.id}`} className="text-xs font-semibold text-primary hover:underline">Export employee data</a></div><ExamsList userId={employee.id} existingByModule={existingByModule} readOnly /></section></main></div>;
}
