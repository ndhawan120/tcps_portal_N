...
export default async function ManagerPage() {
  ...
  const [{ data: allObjectives }, { data: allExams }, { data: pending }] = await Promise.all([
    ...
    teamIds.length
      ? supabase.from("exams").select("user_id,status,result").in("user_id", teamIds)
      : Promise.resolve({ data: [] as any[] }),
    teamIds.length
      ? supabase.from("per_objectives").select("id,user_id,objective_number,title,submitted_at").in("user_id", teamIds).eq("status", "pending_approval").order("submitted_at", { ascending: true })
      : Promise.resolve({ data: [] as any[] }),
  ]);
  ...
  for (const row of allExams ?? []) {
    ...
    if (row.status === "passed" || row.result?.toLowerCase() === "pass") {
      examsPassedByUser[row.user_id] = (examsPassedByUser[row.user_id] ?? 0) + 1;
    }
  }
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-1">
          <div>
            ...
            <p className="text-sm text-on-surface-variant">{isAdmin ? "Overall reporting across employees." : "Only your immediate team is included."}</p>
          </div>
            ...
            {(pending ?? []).map((item) => {
              ...
              return (
                <div key={item.id} className="px-5 py-3 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-medium text-on-surface">{employee ? `${employee.first_name} ${employee.last_name}` : "Employee"}</p>
                    <p className="text-xs text-on-surface-variant">Objective {item.objective_number}: {item.title}</p>
                  </div>
                  <Link href="/approvals" className="text-xs font-semibold text-primary hover:underline">Review →</Link>
                </div>
              );
            })}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface-container text-on-surface-variant text-xs uppercase font-semibold">
                <tr><th className="text-left px-5 py-3">Name</th><th className="text-left px-5 py-3">PER Progress</th><th className="text-left px-5 py-3">Exams Passed</th><th className="px-5 py-3"></th></tr>
              </thead>
