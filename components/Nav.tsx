...
export default function Nav({ role, name }: { role: string; name: string }) {
  ...
  const links = useMemo<NavLink[]>(() => {
    if (role === "admin") {
      return [
        ...SHARED_LINKS,
        { href: "/manager", label: "Team", icon: "groups" },
  ...
  const navLink = (link: NavLink) => (
    <Link
      key={link.href}
      href={link.href}
      className={`flex items-center gap-3 rounded-md px-3 py-3 text-sm font-semibold transition ${isActive(link.href) ? "bg-primary text-white" : "text-white/75 hover:bg-white/10 hover:text-white"}`}
    >
      <span className="material-symbols-outlined w-5 shrink-0 text-center text-[21px]" aria-hidden="true">{link.icon}</span>
    ...
    <>
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 bg-[#191919] text-white lg:flex lg:flex-col border-r border-black/20">
        ...
        <div className="mt-auto px-4 pb-4">
          <Link href="/exams" className="block w-full bg-white text-black py-3 rounded-lg text-center text-xs font-bold">New Exam Entry</Link>
      ...
      <div className="lg:hidden sticky top-0 z-50 bg-[#191919] text-white px-4 py-3 flex items-center justify-between">
        <Link href="/dashboard" className="font-extrabold text-primary">TCPS</Link>
        <div className="flex items-center gap-2"><NotificationBell /><button type="button" aria-label="Toggle navigation" onClick={() => setMobileOpen(v => !v)} className="h-9 w-9 border border-white/15 rounded-md">{mobileOpen ? "×" : "☰"}</button></div>
      </div>
      {mobileOpen && <div className="lg:hidden fixed inset-0 z-40 bg-black/40" onClick={() => setMobileOpen(false)} />}
      <div className={`lg:hidden fixed left-0 right-0 top-[60px] z-50 bg-[#191919] px-3 pb-4 pt-3 shadow-xl ${mobileOpen ? "block" : "hidden"}`}>
        <nav className="space-y-1">{links.map(navLink)}<Link href="/exams" className="block mt-2 rounded-md bg-white px-3 py-3 text-center text-sm font-bold text-black">New Exam Entry</Link><button type="button" onClick={logout} className="w-full rounded-md px-3 py-3 text-left text-sm font-semibold text-white/70">Log out</button></nav>
      </div>
      ...
      <header className="hidden lg:flex fixed top-0 left-64 right-0 z-40 h-[68px] bg-[#fafafa] border-b border-[#e7bdb2] items-center justify-between px-10">
        <form onSubmit={submitSearch} className="w-full max-w-xl rounded-xl bg-[#f0f0f0] px-4 py-2.5 flex items-center gap-3">
          <span className="material-symbols-outlined text-[#6b6b6b] text-[21px]" aria-hidden="true">search</span>
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-transparent border-0 outline-none text-sm" placeholder="Search exams or documents..." aria-label="Search exams or documents" />
        </form>
