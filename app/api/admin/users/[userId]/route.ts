...
function adminClient() { const key = process.env.SUPABASE_SERVICE_ROLE_KEY; const url = process.env.NEXT_PUBLIC_SUPABASE_URL; if (!key || !url) throw new Error("Server administration is not configured. Add SUPABASE_SERVICE_ROLE_KEY to the server environment."); return createSupabaseAdmin(url, key, { auth: { autoRefreshToken: false, persistSession: false } }); }
...
export async function PATCH(request: Request, { params }: { params: { userId: string } }) {
  ...
  let body: { first_name?: string; last_name?: string; department?: string | null; joining_date?: string | null; date_of_birth?: string | null; password?: string };
  try { body = await request.json(); } catch { return NextResponse.json({ error: "Invalid request." }, { status: 400 }); }
  const first_name = body.first_name?.trim() ?? ""; const last_name = body.last_name?.trim() ?? "";
  ...
  try {
    ...
    const { error } = await admin.from("profiles").update({ first_name, last_name, department: body.department?.trim() || null, joining_date: body.joining_date || null, date_of_birth: body.date_of_birth || null }).eq("id", params.userId);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    if (body.password) { const { error: passwordError } = await admin.auth.admin.updateUserById(params.userId, { password: body.password }); if (passwordError) return NextResponse.json({ error: passwordError.message }, { status: 400 }); }
...
export async function DELETE(_request: Request, { params }: { params: { userId: string } }) {
  ...
  try {
    ...
    const { error } = await admin.auth.admin.deleteUser(params.userId); if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
