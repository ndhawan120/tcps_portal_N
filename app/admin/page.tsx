...
export default async function AdminPage() {
  ...
  const people = users ?? [];
  const totalMembers = people.length; const activeMembers = people.filter((u) => u.status === "active").length;
  const totalAdmins = people.filter((u) => u.role === "admin").length; const activeAdmins = people.filter((u) => u.role === "admin" && u.status === "active").length;
  const totalManagers = people.filter((u) => u.role === "manager").length; const activeManagers = people.filter((u) => u.role === "manager" && u.status === "active").length;
  const totalEmployees = people.filter((u) => u.role === "employee").length; const activeEmployees = people.filter((u) => u.role === "employee" && u.status === "active").length;
  ...
  const passedExams = (exams ?? []).filter((e) => e.result === "pass").length; const examCount = (exams ?? []).length; const examPassRate = examCount ? Math.round((passedExams / examCount) * 100) : 0;
  const approvedPER = (objectives ?? []).filter((o) => o.status === "approved").length; const perCount = (objectives ?? []).length; const perApprovalRate = perCount ? Math.round((approvedPER / perCount) * 100) : 0;

  let lastSignIn: Record<string, string | null> = {};
  try { const key = process.env.SUPABASE_SERVICE_ROLE_KEY; const url = process.env.NEXT_PUBLIC_SUPABASE_URL; if (key && url) { const admin = createSupabaseAdmin(url, key, { auth: { autoRefreshToken: false, persistSession: false } }); const { data } = await admin.auth.admin.listUsers({ page: 1, perPage: 1000 }); for (const authUser of data.users) lastSignIn[authUser.id] = authUser.last_sign_in_at ?? null; } } catch { lastSignIn = {}; }
  ...
  return <div><RealtimeRefresh /><Nav role="admin" name={`${profile.first_name ?? ""} ${profile.last_name ?? ""}`} /><main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <section className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 mt-6"><h2 className="text-lg font-bold text-on-surface">Administration</h2><p className="text-sm text-on-surface-variant mt-1 mb-5">Choose the operational or configuration area instead of duplicating detailed records here.</p><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"><AdminLink href="/team" title="Workforce / Team" description="View active employee progress, team relationships and people needing attention." primary /><AdminLink href="/employees" title="People" description={`View all ${totalMembers} office members: ${totalEmployees} employees, ${totalManagers} managers and ${totalAdmins} admins.`} /><AdminLink href="/admin/roles" title="Roles & Access" description="Manage custom role names and active departments." /><AdminLink href="/admin/branding" title="Branding" description="Manage the TC Professional Services logo." /><AdminLink href="/approvals" title={`PER Approvals (${pendingPER ?? 0})`} description="Review organisation-wide PER submissions." /><AdminLink href="/reports" title="Reports" description="Analyse organisation-wide KPIs and distributions." /></div></section>
  </main></div>;
}
