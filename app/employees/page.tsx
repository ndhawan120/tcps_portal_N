...
export default async function EmployeesPage() {
  ...
  for (const objective of objectives ?? []) {
    (objectivesByUser[objective.user_id] ??= []).push(objective);
  }
  ...
  const employeeRows = employeeList.map((employee) => {
    ...
    const approvedPER = employeeObjectives.filter(
      (item) => item.status === "approved"
    ).length;
    const pendingPER = employeeObjectives.filter(
    ...
    return {
      ...
      pendingPER,
      passedExams,
      perProgress: Math.round((approvedPER / TOTAL_OBJECTIVES) * 100),
      examProgress: Math.round((passedExams / TOTAL_EXAMS) * 100),
    };
  });
  ...
  const pendingPERApprovals = employeeRows.reduce(
    (total, employee) => total + employee.pendingPER,
    0
  );
    ...
    <div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div>
            ...
            <p className="text-sm text-on-surface-variant mt-1">
              {profile.role === "manager"
                ? "View only your immediate team and their progress."
                : "View employee information and company-wide progress."}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <StatCard label="Pending PER Approvals" value={pendingPERApprovals} />
        </div>
          ...
          {employeeRows.length === 0 ? (
            <div className="px-6 py-12 text-center">
              ...
              <p className="text-xs text-on-surface-variant mt-2">
                {profile.role === "manager"
                  ? "Assign employees to this manager from the Admin panel."
                  : "Employee accounts will appear here automatically."}
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-surface-container text-xs uppercase text-on-surface-variant">
                  <tr>
                    ...
                    <th className="text-left px-5 py-3">PER</th>
                    <th className="text-left px-5 py-3">Exams</th>
                ...
                <tbody className="divide-y divide-outline-variant">
                  {employeeRows.map((employee) => (
                    <tr key={employee.id} className="hover:bg-surface-container-low">
